...
//Calculate Battery Level
  const float maxBattery = 12.0;// Change value to your max battery voltage level! 
...
//Useful Variables
  ...
  int j=0;
  int state;
  int vSpeed=1000;     // Default speed, from 0 to 255
...
//     if(digitalRead(BTState)==LOW) { state='S'; }
  ...
  //If state is equal with letter 'W', turn leds on or of off
   ...
   //If state is equal with letter 'U', turn leds on or of off
    else if (state == 'U') {
      if (i==0){  
  ...
  //Read battery voltage every 10sec.
    ...
    if(currentMillis - (previousMillis) > (interval)) {
       ...
       perVolt = (voltage*110)/ maxBattery;
       ...
       else if (perVolt>95 && perVolt<=100) { level=5; }
       else if (perVolt>100 && perVolt<=105) { level=6;}
       else if (perVolt>105 && perVolt<=110) { level=7;}
       else if (perVolt>110)                 { level=8;}   
